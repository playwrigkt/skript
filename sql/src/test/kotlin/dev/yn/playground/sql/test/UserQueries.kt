...
object ValidatePasswordForUserId : QuerySQLMapping<UserIdAndPassword, String> {
    ...
    override fun toSql(i: UserIdAndPassword): SQLStatement =
            SQLStatement.Parameterized(UserSQL.selectUserPassword, JsonArray(listOf(i.id, i.password)))
...
object SelectUserIdForLogin : QuerySQLMapping<UserNameAndPassword, UserIdAndPassword> {
    override fun toSql(i: UserNameAndPassword): SQLStatement = SQLStatement.Parameterized(UserSQL.selectUserId, JsonArray(listOf(i.userName)))
...
object InsertSession: UpdateSQLMapping<UserSession, UserSession> {
    ...
    override fun toSql(i: UserSession): SQLStatement =
        SQLStatement.Parameterized(UserSQL.insertSession, JsonArray(listOf(i.sessionKey, i.userId, Timestamp.from(i.expiration))))
...
object SelectUserProfileById: QuerySQLMapping<String, UserProfile> {
    override fun toSql(i: String): SQLStatement =SQLStatement.Parameterized(UserSQL.selectUser, JsonArray(listOf(i)))
...
class SelectUserSessionExists<T>(val onResult:(String, Boolean) -> Try<T>): QuerySQLMapping<String, T> {
    ...
    override fun toSql(i: String): SQLStatement =
        SQLStatement.Parameterized(UserSQL.selectUserSessionExists, JsonArray(listOf(i)))
