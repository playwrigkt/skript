...
fun <I, O, O2, C: SQLTaskContext<*>> Skript<I, O, C>.query(mapping: SQLMapping<O, O2, SQLCommand.Query, SQLResult.Query>): Skript<I, O2, C> =
        this.andThen(SQLSkript.query(mapping))

fun <I, O, O2, C: SQLTaskContext<*>> Skript<I, O, C>.update(mapping: SQLMapping<O, O2, SQLCommand.Update, SQLResult.Update>): Skript<I, O2, C> =
        this.andThen(SQLSkript.update(mapping))

fun <I, O, O2, C: SQLTaskContext<*>> Skript<I, O, C>.exec(mapping: SQLMapping<O, O2, SQLCommand.Exec, SQLResult.Void>): Skript<I, O2, C> =
        this.andThen(SQLSkript.exec(mapping))

sealed class SQLSkript<IN, OUT>: Skript<IN, OUT, SQLTaskContext<*>> {
