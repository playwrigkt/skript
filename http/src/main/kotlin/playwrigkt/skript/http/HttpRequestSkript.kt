...
class HttpRequestSkript: Skript<HttpClientRequest, HttpClientResponse, HttpRequestTroupe> {
    ...
    data class RequestMapping<I, Troupe>(val method: HttpMethod,
                                         ...
                                         val body:Skript<I, ByteArray, Troupe> = Skript.map { ByteArray(0) }): Skript<I, HttpClientRequest, Troupe> {
        override fun run(i: I, troupe: Troupe): AsyncResult<HttpClientRequest> {
            ...
            return uriResult.flatMap { uri ->
                    ...
                    queryParametersResult.map { queryParameters ->
                        HttpClientRequest( method, uri, pathParameters, queryParameters, headers, bodyFuture)
                    } } } }
        }
    }

    data class ResponseMapping<O, Troupe>(val mappers: List<Pair<IntRange, Skript<HttpClientResponse, O, Troupe>>>): Skript<HttpClientResponse, O, Troupe> {

        override fun run(i: HttpClientResponse, troupe: Troupe): AsyncResult<O> =
            mappers
                    .fold(Option.empty(), firstMatch(i))
                    .map { it.run(i, troupe) }
                    .getOrElse { AsyncResult.failed(HttpError.Client.UnhandledResponse(i)) }

        private fun firstMatch(httpClientResponse: HttpClientResponse): (Option<Skript<HttpClientResponse, O, Troupe>>, Pair<IntRange, Skript<HttpClientResponse, O, Troupe>>) -> Option<Skript<HttpClientResponse, O, Troupe>> = {
            skript, candidate ->
            skript.orElse {
