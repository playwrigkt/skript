...
object GetChatRoom: SQLQueryMapping<String, playwright.skript.chatrooom.models.ChatRoom> {
    val selectChatroom = """
    ...
    override fun mapResult(i: String, rs: SQLResult.Query): Try<playwright.skript.chatrooom.models.ChatRoom> {
        return Try { rs.result.next() }
                .flatMap(this::parseFirstRow)
                .flatMap { rs.result.asSequence().fold(Try.Success(it)) { agg: Try<playwright.skript.chatrooom.models.ChatRoom>, nextRow: SQLRow -> agg.map { chatRoom ->
                    chatRoom.copy(
    ...
    private fun parseFirstRow(row: SQLRow): Try<playwright.skript.chatrooom.models.ChatRoom> {
        return Try {
            playwright.skript.chatrooom.models.ChatRoom(
                    id = row.getString("id"),
    ...
    private fun parseUser(row: SQLRow): Try<playwright.skript.chatrooom.models.ChatRoomUser> {
        return Try {
    ...
    private fun addTo(users: Set<playwright.skript.chatrooom.models.ChatRoomUser>, chatRoomUser: playwright.skript.chatrooom.models.ChatRoomUser):Set<playwright.skript.chatrooom.models.ChatRoomUser> {
        return users.plus(chatRoomUser).fold(emptyMap<String, playwright.skript.chatrooom.models.ChatRoomUser>()) { agg, nextUser ->
