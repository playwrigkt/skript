...
class SelectSessionByKey<T>(val validateSesssion: (UserSession, T) -> Try<T>): SQLQueryMapping<playwright.skript.auth.TokenAndInput<T>, T> {
    override fun mapResult(i: playwright.skript.auth.TokenAndInput<T>, rs: SQLResult.Query): Try<T> =
        Try { rs.result.iterator().next() }
                .map {  UserSession(
