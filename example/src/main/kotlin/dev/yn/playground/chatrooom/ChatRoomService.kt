...
class ChatroomService(val provider: ApplicationContextProvider) {
    companion object {
        val addUserTask: Task<ChatRoomUser, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.addUserTransaction)
        val removeUserTask: Task<ChatRoomUser, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.removeUserTransaction)
        val addUserPermissionsTask: Task<ChatRoomUser, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.addUserPermissions)
        val removeUserPermissionsTask: Task<ChatRoomUser, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.removeUserPermissions)
        val addPublicPermissionTask: Task<ChatRoomPermissions, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.addPermissions)
        val removePublicPermissionTask: Task<ChatRoomPermissions, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.removePermissions)
        val createChatRoomTask: Task<ChatRoom, ChatRoom, ApplicationContext<UserSessionCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.createChatRoomTransaction)
        val updateChatRoomTask: Task<ChatRoom, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.updateChatRoomTransaction)
        val getChatroomTask: Task<String, ChatRoom, ApplicationContext<ChatroomOperationCache>> = SQLTransactionTask.transaction(ChatRoomTransactions.getChatRoomTransaction)
    }
