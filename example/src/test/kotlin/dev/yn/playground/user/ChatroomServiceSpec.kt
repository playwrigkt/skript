...
class ChatroomServiceSpec : StringSpec() {
    ...
    companion object {
        ...
        val createChatRoom by lazy { UnpreparedTransactionalSQLTask(ChatRoomTransactions.createChatRoomTransaction).prepare(provider) }
        val getChatRoom by lazy { UnpreparedTransactionalSQLTask(ChatRoomTransactions.getChatRoomTransaction).prepare(provider) }
        val addUser by lazy { UnpreparedTransactionalSQLTask(ChatRoomTransactions.addUserTransaction).prepare(provider) }
        val deleteUser by lazy { UnpreparedTransactionalSQLTask(ChatRoomTransactions.removeUserTransaction).prepare(provider) }
        val addPublicPermissions by lazy { UnpreparedTransactionalSQLTask(ChatRoomTransactions.addPermissions).prepare(provider) }
        val removePublicPermissions by lazy { UnpreparedTransactionalSQLTask(ChatRoomTransactions.removePermissions).prepare(provider) }
    }
    ...
    init {
        "create a chatroom" {
            ...
            val chatRoomWithNewUser = chatRoom.copy(users = chatRoom.users.plus(ChatRoomUser(Reference.Defined(user3.userProfile.id, user3.userProfile), Reference.Empty(chatRoomId), setOf(ChatRoomPermissionKey.Get.key, ChatRoomPermissionKey.Update.key))))
            awaitSucceededFuture(
                    addUser.run(TokenAndInput(session.sessionKey, ChatRoomUser(Reference.Defined(user3.userProfile.id, user3.userProfile), Reference.Empty(chatRoomId), setOf(ChatRoomPermissionKey.Get.key, ChatRoomPermissionKey.Update.key)))),
                    chatRoomWithNewUser)
            ...
            awaitSucceededFuture(
                    addPublicPermissions.run(TokenAndInput(session.sessionKey, ChatRoom(chatRoomId, "", "", emptySet(), setOf(ChatRoomPermissionKey.AddUser.key)))),
            ...
            awaitSucceededFuture(
                    removePublicPermissions.run(TokenAndInput(session.sessionKey, ChatRoom(chatRoomId, "", "", emptySet(), setOf(ChatRoomPermissionKey.Get.key, ChatRoomPermissionKey.AddUser.key)))),
                    chatRoomAfterDeleteUser.copy(publicPermissions = emptySet()))
        }
    }
